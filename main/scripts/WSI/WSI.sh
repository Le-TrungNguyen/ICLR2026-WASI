...
for truncation_threshold in "${truncation_thresholds[@]}"; do
  ...
  common_methods="--model.with_WSI True --model.truncation_threshold $truncation_threshold"
  ...
  for model in "${models[@]}"; do
    ...
    for dataset in "${!datasets[@]}"; do
      ...
      specific_logger_args="--logger.save_dir ${saved_location}/$model/$dataset/WSI/var${truncation_threshold}/finetuning"
      ...
      for i in "${!num_of_finetune_list[@]}"; do
        ...
        python trainer.py ${all_args} --model.backbone $model --logger.exp_name WSI_var${truncation_threshold}_l${num_of_finetune} --model.num_of_finetune $num_of_finetune
      done
    done
  done
done
