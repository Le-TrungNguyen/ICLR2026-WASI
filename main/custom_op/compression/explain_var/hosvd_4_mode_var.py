...
def truncated_svd_4_mode_var(X, var=0.9, return_rank=False, return_full_rank=False, rank=None):
    """
...
def svd_mode_n(n, A, var=0.9, return_full_rank=False, return_rank=False, rank=None):
    """
    ...
    return truncated_svd_4_mode_var(unfolded_A, var, return_full_rank=return_full_rank, return_rank=return_rank, rank=rank)


def hosvd_4_mode_var(A, var=0.9, return_rank=False, return_full_rank=False, rank=None):
    """
    ...
    if not return_rank and not return_full_rank:
        for i in range(A.dim()):
            u, _, _ = svd_mode_n(i, A, var, rank=rank[i])
        ...
        rank_list = []
        for i in range(A.dim()):
            u, _, _, r = svd_mode_n(i, A, var, return_rank=True, rank=rank[i])
            S = th.tensordot(S, u, dims=([0], [0]))
            u_list.append(u)
            rank_list.append(r)
        ...
        ex_var_list = []
        for i in range(A.dim()):
            u, _, _, r = svd_mode_n(i, A, var, return_full_rank=True, rank=rank[i])
            S = th.tensordot(S, u, dims=([0], [0]))
            u_list.append(u)
            ex_var_list.append(r)
        return S, u_list, ex_var_list

def restore_hosvd_4_mode(S, u_list):
    """
